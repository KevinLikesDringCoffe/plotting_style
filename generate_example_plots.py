...
# Example 3: Two-column multi-series bar plot (grayscale)
fig, ax = plt.subplots(figsize=(7.0, 4.5))
...
hatches = ['', '///', '\\\\\\', 'xxx']
labels_legend = ['Metric 1', 'Metric 2', 'Metric 3', 'Metric 4']

width = 0.2
x = np.arange(len(labels))

for i, (data, color, hatch, label) in enumerate(zip(series_list, colors, hatches, labels_legend)):
    offset = width * (i - len(series_list)/2 + 0.5)
    ax.bar(x + offset, data, width, label=label,
           color=color, edgecolor='black', linewidth=1.2, hatch=hatch)
...
# Example 4: Single-column scatter plot with text x-labels
fig, ax = plt.subplots(figsize=(3.5, 2.8))
